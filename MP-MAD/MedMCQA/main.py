...
from langchain.llms import HuggingFaceHub,  HuggingFaceHub, HuggingFaceEndpoint
...
df = pd.read_csv('medQ_dataset.csv')
...
for i, round in enumerate([result_round0, result_round1, result_round2, result_round3]):
    df_before_analysis = pd.DataFrame(round, columns = ['AI_label', 'correct_label'])
    df_result, bool_counts = science_analysis(df_before_analysis)
    df_result.to_csv('result_round'+str(i)+'_after_science_analysis.csv', index=False)


    print(f'''Results After Analysis: Round_{i}''')
    print(df_result)
    print(df_result['comparison_result'].value_counts(dropna=False))
    print('\n \n \n')
